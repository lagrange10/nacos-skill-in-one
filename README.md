...
| `adopt <skill>` | 将本机 Skill 纳入唯一信源、建立 Junction，并默认校验、提交和推送 |
| `sync` | 拉取远端，建立或刷新本机 Junction 并校验 |
| `rename <old> <new>` | 同步迁移仓库目录、frontmatter 和本机 Junction |

不带模式时，由 Agent 根据语境判断是上传、纳管、接入还是迁移；只有单纯修改内容而未表达发布意图时，不自动推送。明确使用 `adopt <skill>` 是例外：它会默认完成有限范围的 Git 提交与推送；只有明确说“仅本地纳管”或“不要上传”才保留本地变更。
